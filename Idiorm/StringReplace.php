 ...
 * @example String::strReplaceOutsideQuotes('?', '%s', 'columnA = "Hello?" AND columnB = ?');
 * @example String::value('columnA = "Hello?" AND columnB = ?')->replaceOutsideQuotes('?', '%s');
...
class StringReplace
{
    ...
    public function replaceOutsideQuotes($search, $replace)
    {
        ...
        return $this->stringReplaceOutsideQuotes();
    }
     ...
     * @return string
     */
    protected function stringReplaceOutsideQuotes()
    {
        $re_valid = '/
        ...
        if (!preg_match($re_valid, $this->subject)) {
            throw new StringReplaceException("Subject string is not valid in the replaceOutsideQuotes context.");
        }
        $re_parse = '/
                ...
                /sx';
        return preg_replace_callback($re_parse, [$this, 'strReplaceOutsideQuotesCb'], $this->subject);
    }
     ...
     * @return string
     */
    protected function strReplaceOutsideQuotesCb($matches)
    {
